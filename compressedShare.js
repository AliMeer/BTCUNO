...
generateShares = () => {
	...
	threshold = threshold;
	const shares = secrets.share(stringToHex(mnemonic), n, threshold);
...
recoverFromShares = decryptedShares => {
	if (decryptedShares.length >= threshold) {
		...
		throw new Error(`supplied number of shares are less than the threshold (${threshold})`);
	}
};
	...
	if (existingShares.length) {
		for (const share of existingShares) {
			if (share.meta.walletID === decryptedShare.meta.walletID) {
				throw new Error('You cannot store multiple shares from a single user.');
			}
		}
	}
...
addMeta = (encryptedShare, tag) => {
	...
	console.log(`original object: ${share} length: ${share.length}`);
	console.log(`packed object: ${msgpack.pack(share)} length: ${msgpack.pack(share).length}`);
	return JSON.stringify(share);
	//return msgpack.pack(share);
};
...
createQRShare = encryptedShare => {
	const tag = makeRandomWord(5);
	const metaShare = addMeta(encryptedShare, tag);
	return metaShare;
...
function makeRandomWord(length) {
	...
	for (var i = 0; i < length; i++) OTP += possible.charAt(Math.floor(Math.random() * possible.length));
	return OTP;
}
...
//     //Converting to 16bit charactor string
//     console.log(`\nshare ${i+1}, with metadata: ` + createQRShare(shares[i]));
// }
const s = createQRShare(shares[0]);
const packedS = msgpack.pack(s);

console.log(`\nORIGINAL share is: `, s, Buffer.byteLength(s, 'utf8'));
console.log(`\npacked share is: `, packedS);
console.log(`\npacked byteLength is: `, packedS.byteLength);
console.log(`\nUNpacked share is: `, msgpack.unpack(packedS));

console.log(`\n\nUsing zlib - using deflate\n`);
zlib.deflate(s, function(err, buf) {
	console.log(`deflated bytelength: ${buf.byteLength}`);
	console.log('deflated share:', buf.toString('hex'));
	console.log(`error is ${err}`);
	var path = './zlib.png';
	QRCode.toFile(path, buf.toString('hex'), function(err) {
		if (err) throw err;
	});
});

/*
console.log(`\n\nUsing zlib - using gzip\n`);
zlib.gzip(s, function(err, buf) {
	console.log(`gzip bytelength: ${buf.byteLength}`);
	console.log('gzip share:', buf.toString('hex'));
	console.log(`error is ${err}`);
	var path = './gzip.png';
	QRCode.toFile(path, buf.toString('hex'), function(err) {
		if (err) throw err;
